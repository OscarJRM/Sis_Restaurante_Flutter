...
import 'package:sistema_restaurante/pages/carritoCustom2.dart';
...
class carritoCustom1 extends StatefulWidget {
  ...
  const carritoCustom1({required this.detallePedido, super.key});
...
class _carritoCustom1State extends State<carritoCustom1> {
  ...
  Future<void> _fetchData() async {
    ...
    final result = await conn.execute(
        "SELECT * from Productos where ID_PRO=\$1",
        parameters: [widget.detallePedido.idProPed]);
    if (mounted) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    final globalState = Provider.of<GlobalState>(context, listen: false);
    if (listaPlatoC.isNotEmpty) {
      return carritoCustom2(
        detallePedido: widget.detallePedido,
        plato: listaPlatoC[0],
      );
    } else {
      // Puedes mostrar un indicador de carga o un mensaje de error aquí
      return CircularProgressIndicator(); // o cualquier otro widget que desees
    }
  }
}
/*
